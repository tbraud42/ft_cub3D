...
/*   Updated: 2025/01/06 15:24:50 by brguicho         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	ft_spread(char **arr, int x, int y, int nbr_line)
{
	...
	j = x;
	arr[i][j] = '2';
	if (i > 0 && arr[i - 1][j] != '1' && arr[i - 1][j] != '2' && arr[i][j + 1] != '\0')
		ft_spread(arr, x, y - 1, nbr_line);
	if (i < nbr_line
		&& arr[i + 1][j] != '1' && arr[i + 1][j] != '2' && arr[i][j + 1] != '\0')
		ft_spread(arr, x, y + 1, nbr_line);
	if (j > 0 && arr[i][j - 1] != '1' && arr[i][j - 1] != '2' && arr[i][j + 1] != '\0')
		ft_spread(arr, x - 1, y, nbr_line);
	if (j < ft_strlen(arr[i]) - 1 && arr[i][j + 1] != '1' && arr[i][j + 1] != '2' && arr[i][j + 1] != '\0')
		ft_spread(arr, x + 1, y, nbr_line);
	return ;
}
